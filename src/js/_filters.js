...
function categoryFilter(category) {
    let shelvesWithCat = booksOnShelves.shelves.filter((item) =>
        item.categories.includes(category)
    );
    const shelvesWithCatByID = shelvesWithCat.map(({ shelfID }) => shelfID);
    let catBooks = [];
    booksOnShelves.books.forEach((element) => {
        const master = element.shelves.find(
            (item) => item.shelf == "masterShelf"
        );
        if (master) {
            if (
                master.details.hasOwnProperty("categories") &&
                master.details.categories.includes("kids")
            ) {
