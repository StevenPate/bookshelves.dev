...
const processFoldersToShelves = (folderPaths) => {
    ...
    if (
        ...
        fs.createReadStream(bookshelves.localInventoryFile)
            .pipe(csv())
            .on("data", (data) =>
                localInventory.push({ ...data, useLocalInventory })
            )
            .on("end", () => {
    ...
    const createShelves = (folderPath) => {
        ...
        for (let file in folderContents) {
            const fileType =
                folderPath == masterShelf ? "masterShelf" : "shelf";
            ...
            const createShelf = (fileData, filePath, fileType) => {
                ...
                books.forEach((book) =>
                    createShelfEntry(book, filePath, fileType)
                );
            };
            ...
            if (ext == ".json" && name == "test") {
                ...
                lists.map((shelf) =>
                    createShelf([shelf], shelf.shelfID, "shelf")
                );
            }
            // if (ext == '.csv') {
...
const bookshelvesToJSON = (itemsForJSON) => {
    ...
    fs.writeFile(bookshelves.dataFile, JSONdata, "utf8", (err) => {
        if (err) {
            ...
            console.log(
                `/||||| ${bookshelves.dataFile} is written successfully!`
            );
        }
    });
};
